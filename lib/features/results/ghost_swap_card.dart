...
class GhostSwapCard extends ConsumerWidget {
              ...
              children: [
                Expanded(
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      if (!proposal.comparisonAvailable || proposal.priceDifference == null)
                        _buildPricingUnavailableBanner(proposal)
                      else
                        if (proposal.priceDirection == 'savings')
                          Container(
                            ...
                            child: Text(
                              'Switch Savings\nHealthier & Save \$${proposal.priceDifference!.abs().toStringAsFixed(2)} at ${proposal.storeLocation != null ? proposal.storeLocation!.split(' ').first : 'Store'}!',
                              style: const TextStyle(
                        ...
                        else if (proposal.priceDirection == 'loss')
                          Text(
  ...
  Widget _buildPricingUnavailableBanner(SwapProposal proposal) {
    // If a network/auth/store failure caused the null price, show a specific
    // message. Otherwise show the standard package-size note.
    final hasNetworkFailure = proposal.storeLocation == null &&
        proposal.pricingFailure != null;

    final message = hasNetworkFailure
        ? proposal.pricingFailure!.userMessage
        : 'Price Comparison Unavailable\n'
            '${proposal.comparisonReason ?? 'Package sizes are not comparable'}';

    final icon = hasNetworkFailure
        ? Icons.wifi_off_outlined
        : Icons.info_outline;

    final color = hasNetworkFailure
